...
三服务 AI 云盘：Go 后端负责认证、对象存储与请求代理，Python Agent 负责工具编排与文件检索，React 提供网盘与流式对话界面。核心能力是 Agent 编排与混合检索 RAG，文件不只是存起来，还能被检索、被阅读、被回答。
...
- **多服务边界与安全**：JWT + 内部 agent token 轮换双认证；Go HTTP middleware 区分调用方角色（JWT→user / X-Agent-Token→agent），写内容接口仅 agent 可用；用户 LLM 密钥 AES-GCM 加密落库，按请求解密后注入，前端不落明文；Go 代理层做 SSE 流式透传与并发限流。把业务域和 Agent 拆成两个服务，是为了让认证、存储与模型编排独立演进，敏感配置不需要下沉到 Agent。
...
```bash
# backend/
go run ./cmd/cloud-drive        # dev，默认 :8080
go test ./...
go vet ./...
```
